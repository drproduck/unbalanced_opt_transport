...
# for n_pts in [2]:
    ...
    for i in range(n_iter):
        ratio2.append((f_val_list[i] - f_val_list[-1]) / grad_norm_list[i])
    max_ratio_list.append(np.max(ratio2))
    argmax_ratio_list.append(np.argmax(ratio2))

# print(argmax_ratio_list)
# plt.plot(np.arange(n_iter), ratio2)
# plt.title('$(f(u^k,v^k) - f(u^*,v^*)) / (||du f(u^k, v^k)||_1 + ||dv f(u^k, v^k||_1)$')
# plt.plot(np.arange(1, 100), max_ratio_list)
plt.plot(np.arange(n_iter), unreg_obj_list)
